...
export async function initDb() {
  ...
  const [existingMembers] = await pool.execute('SELECT full_name FROM youth_members');
  const existingMemberNames = new Set(existingMembers.map(r => r.full_name.toLowerCase()));
  for (const fullName of MEMBERS) {
    if (!existingMemberNames.has(fullName.toLowerCase())) {
      await pool.execute('INSERT INTO youth_members (full_name) VALUES (?)', [fullName]);
    }
  }
  ...
  const [existingInterests] = await pool.execute('SELECT slug FROM santa_interests');
  const existingSlugs = new Set(existingInterests.map(r => r.slug));
  for (const [slug, label, category] of INTERESTS) {
    if (!existingSlugs.has(slug)) {
      await pool.execute('INSERT INTO santa_interests (slug, label, category) VALUES (?, ?, ?)', [slug, label, category]);
    }
  }
