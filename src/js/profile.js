...
async function loadSantaMatch() {
  const container = document.getElementById('santaContent');
  ...
  try {
    ...
    if (!data.match) {
      document.getElementById('statSanta').textContent = '—';
    ...
    document.getElementById('statSanta').textContent = '1';

    let interestChips = '';
    if (interests && interests.length > 0) {
      interestChips = interests.map(i =>
        `<span class="pf-santa-chip">${i.label}</span>`
      ).join('');
    }

    const descHtml = description
      ? `<div class="pf-santa-desc">"${description}"</div>`
      : '';

    container.innerHTML = `
      <div class="pf-santa-grid">
        <div class="pf-santa-avatar">
          <div class="pf-santa-avatar-ring"></div>
          <span class="pf-santa-avatar-icon">🎁</span>
        </div>
        <div class="pf-santa-info">
          <div class="pf-santa-status matched"><span class="pf-santa-status-dot"></span> Matched</div>
          <div class="pf-santa-label">You are Secret Santa for</div>
          <div class="pf-santa-username">${recipientUsername}</div>
          <div class="pf-santa-interests">${interestChips}</div>
          ${descHtml}
          <a href="/santa-profile" class="pf-btn pf-btn-outline" style="margin-top:0.8em;display:inline-flex;">
    ...
    container.innerHTML = `
      <div class="pf-santa-empty">
        <div class="pf-santa-empty-icon">🎄</div>
        <div class="pf-santa-empty-title">Setup Required</div>
        <p class="pf-santa-empty-text">Set up your Secret Santa profile to get matched!</p>
        <a href="/santa-profile" class="pf-btn pf-btn-gold">Setup Profile</a>
      </div>`;
  }
}
...
function searchSceneHtml() {
  return `
    ...
    <div class="pf-santa-empty">
      <p class="pf-santa-empty-text">A match is revealed automatically as more profiles are added. Save your profile and check back soon!</p>
      <a href="/santa-profile" class="pf-btn pf-btn-gold">Setup Profile</a>
    </div>`;
}
...
async function loadLeaderboard() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/leaderboard/scores?limit=10`);
    const data = await res.json();
    const scores = data.data || [];

    if (scores.length === 0) {
      document.getElementById('statScore').textContent = '—';
      list.innerHTML = '<li class="pf-lb-empty">No scores yet. Play trivia to get on the board!</li>';
      return;
    }
    ...
    document.getElementById('statScore').textContent =
      scores.find(s => s.name === user.name)?.score ?? '—';

    list.innerHTML = scores.map((s, i) => {
      const isMe = s.name === user.name;
      return `
        <li class="pf-lb-item${isMe ? ' is-me' : ''}">
          <span class="pf-lb-rank">${i + 1}</span>
          <span class="pf-lb-name">${s.name}${isMe ? '<span class="pf-lb-you">(You)</span>' : ''}</span>
          <span class="pf-lb-score">${s.score}</span>
        </li>`;
    }).join('');

    gsap.fromTo(list.children, { opacity: 0, x: -10 }, {
      opacity: 1, x: 0, duration: 0.3, stagger: 0.05, ease: 'power2.out',
    });
  } catch {
    document.getElementById('statScore').textContent = '—';
    list.innerHTML = '<li class="pf-lb-empty">Failed to load leaderboard.</li>';
  }
}
...
async function loadEvents() {
  ...
  try {
    ...
    if (!name) {
      ...
      list.innerHTML = '<li class="pf-ev-empty">No events found.</li>';
      return;
    }

    const res = await fetch(`${API_BASE}/api/rsvp/by-name/${encodeURIComponent(name)}`);
    const data = await res.json();
    ...
    if (events.length === 0) {
      list.innerHTML = '<li class="pf-ev-empty">No events registered yet.</li>';
      return;
    }

    list.innerHTML = events.map(e => {
      const { month, day } = formatDate(e.event_date);
      return `
        <li class="pf-ev-item">
          <div class="pf-ev-date">
            <span class="pf-ev-month">${month}</span>
            <span class="pf-ev-day">${day}</span>
          </div>
          <div class="pf-ev-info">
            <div class="pf-ev-name">${e.event_title || e.name || 'Event'}</div>
            <div class="pf-ev-meta">${e.event_date || ''}</div>
          </div>
        </li>`;
    }).join('');

    gsap.fromTo(list.children, { opacity: 0, y: 8 }, {
      opacity: 1, y: 0, duration: 0.3, stagger: 0.05, ease: 'power2.out',
    });
  } catch {
    document.getElementById('statEvents').textContent = '—';
    list.innerHTML = '<li class="pf-ev-empty">Failed to load events.</li>';
  }
}

async function loadInterestsCount() {
  try {
    const profileRes = await fetch(`${API_BASE}/api/santa/profile`, { headers: { Authorization: `Bearer ${token()}` } });
    const profileData = await profileRes.json();
    if (profileData.profile && profileData.profile.interests) {
      const interests = Array.isArray(profileData.profile.interests)
        ? profileData.profile.interests
        : JSON.parse(profileData.profile.interests);
      document.getElementById('statInterests').textContent = interests.length;
    } else {
      document.getElementById('statInterests').textContent = '0';
    }
  } catch {
    document.getElementById('statInterests').textContent = '—';
  }
}
...
async function init() {
  ...
  document.getElementById('greeting').innerHTML = `${firstName}'s <span class="pf-hero-name-accent">Profile</span>`;
  ...
  await Promise.all([loadSantaMatch(), loadLeaderboard(), loadEvents(), loadInterestsCount()]);
}
