...
function showFeedback(el, msg, type) {
  el.textContent = msg;
  el.className = 'sp-feedback is-showing is-' + type;
}
...
function updateProgress() {
  ...
  document.getElementById('progressCount').textContent = `${count} interest${count !== 1 ? 's' : ''}`;
  ...
  document.querySelectorAll('.brutalist-container').forEach(container => {
    ...
    const catSlugs = allInterests.filter(i => i.category === cat).map(i => i.slug);
    const selected = catSlugs.filter(s => selectedSlugs.has(s)).length;
    const countEl = container.querySelector('.brutalist-title-count');
    if (countEl) {
      countEl.textContent = selected > 0 ? `${selected} selected` : 'hover to explore';
    }
  });
}

function updateCharCount() {
  document.getElementById('charCount').textContent = document.getElementById('descriptionInput').value.length;
}

async function loadInterests() {
  try {
    const res = await fetch(`${API_BASE}/api/santa/interests`);
    const data = await res.json();
    allInterests = data.data || [];
    renderInterests();
  } catch {
    document.getElementById('interestsGrid').innerHTML =
      '<div style="color:var(--coral);font-size:0.85em;padding:1em 0;">Failed to load interests</div>';
  }
}
...
function renderInterests() {
  ...
  for (const [cat, items] of Object.entries(categories)) {
    ...
    container.innerHTML = `
      ...
      <div class="brutalist-title">
        <div class="brutalist-title-text">${cat}</div>
        <div class="brutalist-title-count">hover to explore</div>
      </div>
      <div class="brutalist-items">
        ${items.map(interest => `
          <div class="brutalist-item">
            <input type="checkbox" class="brutalist-checkbox" id="int-${interest.slug}"
              data-slug="${interest.slug}" ${selectedSlugs.has(interest.slug) ? 'checked' : ''}>
            <label class="brutalist-label" for="int-${interest.slug}">${interest.label}</label>
          </div>
        `).join('')}
      </div>
    `;

    container.querySelectorAll('.brutalist-checkbox').forEach(cb => {
    ...
    container.querySelectorAll('.brutalist-label').forEach(lbl => {
    ...
    container.addEventListener('click', (e) => {
      if (e.target.closest('.brutalist-items')) return;
  ...
  gsap.fromTo(grid.children, { opacity: 0, y: 12 }, {
...
async function loadProfile() {
  try {
    const res = await fetch(`${API_BASE}/api/santa/profile`, { headers: { Authorization: `Bearer ${token()}` } });
    const data = await res.json();

    if (data.profile) {
      document.getElementById('usernameValue').textContent = data.profile.username;

      if (data.profile.interests) {
        const saved = Array.isArray(data.profile.interests) ? data.profile.interests : JSON.parse(data.profile.interests);
        selectedSlugs = new Set(saved);
        renderInterests();
      }
...
async function saveProfile() {
  ...
  if (selectedSlugs.size === 0) {
    showFeedback(feedbackEl, 'Please select at least one interest.', 'error');
    return;
  }
  ...
  try {
    const res = await fetch(`${API_BASE}/api/santa/profile`, {
      method: 'POST',
      headers: authHeaders(),
      body: JSON.stringify({
        interests: [...selectedSlugs],
    ...
    if (!res.ok) {
      showFeedback(feedbackEl, data.error || 'Failed to save.', 'error');
    ...
    showFeedback(feedbackEl, 'Profile saved!', 'success');
    setLoading(btn, false);
    loadMatchStatus();

    gsap.fromTo('.sp-brutalist-grid', { scale: 1 }, {
      scale: 1.005, duration: 0.15, yoyo: true, repeat: 1, ease: 'power2.out',
    });
  } catch {
    showFeedback(feedbackEl, 'Network error. Please try again.', 'error');
    setLoading(btn, false);
  }
}
...
async function regenerateUsername() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/santa/profile/username`, {
      method: 'PUT',
      headers: authHeaders(),
    });
    const data = await res.json();
    ...
    if (res.ok && data.username) {
      gsap.to(usernameEl, {
        opacity: 0, y: -5, duration: 0.15, onComplete: () => {
          ...
          gsap.to(usernameEl, { opacity: 1, y: 0, duration: 0.2 });
        },
      });
    }
  } catch {}
...
function renderMatchStatus(data) {
  const container = document.getElementById('matchStatus');
  if (!container) return;

  if (!data.match) {
    container.innerHTML = searchSceneHtml();
    return;
  }

  const { recipientUsername, interests, description } = data.match;
  const chips = (interests || []).map(i =>
    `<span class="sp-match-chip">${i.label}</span>`
  ).join('');
  const descHtml = description
    ? `<div class="sp-match-desc">"${description}"</div>`
    : '';

  container.innerHTML = `
    <div class="sp-match-card is-matched">
      <div class="sp-match-grid">
        <div class="sp-match-avatar">
          <div class="sp-match-avatar-ring"></div>
          <span class="sp-match-avatar-icon">🎁</span>
        </div>
        <div class="sp-match-info">
          <div class="sp-match-status matched"><span class="sp-match-status-dot"></span> You're Matched</div>
          <div class="sp-match-label">You are Secret Santa for</div>
          <div class="sp-match-username">${recipientUsername}</div>
          <div class="sp-match-chips">${chips}</div>
          ${descHtml}
        </div>
      </div>
    </div>`;

  gsap.fromTo(container, { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.5, ease: 'power2.out' });
}

function searchSceneHtml() {
  return `
    <div class="sp-match-card is-pending">
      <div class="sp-search-scene">
        <div class="sp-radar" aria-hidden="true">
        ...
        <div class="sp-walk" aria-hidden="true">
        ...
        <p class="sp-search-tag" data-text="Searching for your match..."></p>
      </div>
      <div class="sp-match-empty">
        <p class="sp-match-empty-text">A match is revealed automatically as more profiles are added. Save your profile and check back soon!</p>
      </div>
    </div>`;
}
...
async function loadMatchStatus() {
  try {
    const res = await fetch(`${API_BASE}/api/santa/match/me`, { headers: { Authorization: `Bearer ${token()}` } });
    const data = await res.json();
    renderMatchStatus(data);
  } catch {
    const container = document.getElementById('matchStatus');
    if (container) {
      container.innerHTML = `
        <div class="sp-match-card is-pending">
          <div class="sp-match-empty">
            <div class="sp-match-empty-icon">🎄</div>
            <div class="sp-match-empty-title">Match Status Unavailable</div>
            <p class="sp-match-empty-text">We couldn't check your match status right now. Please try again.</p>
          </div>
        </div>`;
    }
  }
...
async function loadProfileCount() {
  try {
    const res = await fetch(`${API_BASE}/api/santa/profiles`, { headers: { Authorization: `Bearer ${token()}` } });
    const data = await res.json();
    const count = (data.data || []).length;
    document.getElementById('profileCount').textContent = `${count} profile${count !== 1 ? 's' : ''} created so far`;
  } catch {}
}
...
async function init() {
  ...
  if (user.email === 'admin@stmonicayouth.app') {
    document.getElementById('profileCount').innerHTML =
      '<a href="/api/santa/match" id="matchTrigger">Run Matching</a>';
    document.getElementById('matchTrigger')?.addEventListener('click', async (e) => {
      e.preventDefault();
      try {
        const res = await fetch(`${API_BASE}/api/santa/match`, {
          method: 'POST', headers: authHeaders(),
        });
        const data = await res.json();
        if (res.ok) {
          document.getElementById('profileCount').textContent =
            `Matched ${data.matched} of ${data.total} users`;
        }
      } catch {}
    });
  }
  ...
  document.getElementById('regenUsernameBtn').addEventListener('click', regenerateUsername);
  ...
  await Promise.all([loadInterests(), loadProfile(), loadProfileCount(), loadMatchStatus()]);
